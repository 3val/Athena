...
char * ReadFileInformation( const char *szFileLocation, PDWORD pdwReadSize )
{
	HANDLE hFile = NULL;
	...
	hFile = CreateFileA(szFileLocation, GENERIC_READ, NULL, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (!hFile)
	{
		return NULL;
	}
	...
	lpReadBuffer = (char *)malloc(dwFileSize);
	if (!lpReadBuffer)
	{
		CloseHandle(hFile);
	...
	if (!ReadFile(hFile, (LPVOID)lpReadBuffer, dwFileSize, &dwBytesRead, NULL))
	{
		CloseHandle(hFile);
		free(lpReadBuffer);
...
char * GetMD5Hash( const char *szFilePath, char * lpHash, size_t size )
{
	...
	lpFileBuffer = ReadFileInformation(szFilePath, &dwFileSize);
	...
	if(lpFileBuffer != NULL)
	{
		...
		{
			MD5Init(&md5, NULL);
			MD5Update(&md5, (unsigned char *)lpFileBuffer, dwFileSize);
			MD5Final(&md5);
			for(i = 0; i < 16; i++)
			{
				if (!i)
				{
					_snprintf(lpHash, size, "%02X", md5.digest[i]);
				}
				else
				{
					_snprintf(lpHash, size, "%s%02X", lpHash, md5.digest[i]);
				}
			}
		}
...
void StringToHash(const char *szInput, char *szHash, size_t size)
{
	...
	MD5Init(&md5, NULL);
	MD5Update(&md5, (unsigned char *)szInput, lstrlenA(szInput));
	MD5Final(&md5);

	for (int i = 0; i < 16; i++)
	{
		_snprintf(szHash, size, "%s%02X", szHash, md5.digest[i]);
	}
}
